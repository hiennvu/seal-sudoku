...
def seal_inputs(mode):
	"""Construct distorted input for CIFAR training using the Reader ops.
	For counting seals, the mode is the image path
	Returns:
		images: Images. 4D tensor of [batch_size, IMAGE_SIZE, IMAGE_SIZE, 3] size.
		labels: Labels. 1D tensor of [batch_size] size.
	"""
	...
	if mode == 'train':
		filenames = ["train.tfrecords", "val.tfrecords"]
		images, labels = seal_input.inputs(filenames, batch_size=FLAGS.batch_size)
	elif mode == 'test':
		filenames = "test.tfrecords"
		images, labels = seal_input.inputs(filenames, batch_size=FLAGS.batch_size)
	elif mode == 'count':
		filenames = "smalls.tfrecords"
		images, labels = seal_input.count_inputs(filenames, batch_size=FLAGS.batch_size)
